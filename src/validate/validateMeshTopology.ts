 ...
 */
export function validateMeshTopology(mesh: EditableMesh): TopologyValidationResult {
  ...
  if (invalidFaces.length > 0) {
    result.errors.push(`Found ${invalidFaces.length} invalid faces (less than 3 vertices)`);
    result.isValid = false;
  }
...
function findDegenerateFaces(mesh: EditableMesh): number[] {
  ...
  for (const face of mesh.faces) {
    ...
    if (vertices.length === 3) {
      ...
      if (area < 0.000001) { // Very small threshold for numerical precision
...
export function isWatertight(mesh: EditableMesh): boolean {
  ...
  const boundaryEdges = Array.from(edgeUsage.entries())
...
export function getMeshStats(mesh: EditableMesh): {
  ...
  const triangleCount = mesh.faces.filter(face => face.vertexIds.length === 3).length;
  const quadCount = mesh.faces.filter(face => face.vertexIds.length === 4).length;
  const ngonCount = mesh.faces.filter(face => face.vertexIds.length > 4).length;
