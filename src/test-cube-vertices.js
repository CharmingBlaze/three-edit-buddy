...
console.log(`Cube created with ${cube.vertices.length} vertices, ${cube.faces.length} faces, ${cube.edges.length} edges`);
...
if (duplicates.length > 0) {
  ...
  duplicates.forEach(d => {
    console.log(`  Position ${d.position}: Vertex ${d.original} and ${d.duplicate}`);
  });
} else {
  console.log('✅ No duplicate vertices found - vertex sharing is working correctly!');
}
...
console.log(`Moving vertex ${testVertexId} from ${JSON.stringify(originalPosition)}`);
...
console.log(`Vertex ${testVertexId} new position: ${JSON.stringify(updatedVertex.position)}`);

// Count how many faces use this vertex
const facesUsingVertex = cube.faces.filter(face => 
  face.vertexIds.includes(testVertexId)
);
console.log(`Vertex ${testVertexId} is used by ${facesUsingVertex.length} faces`);
