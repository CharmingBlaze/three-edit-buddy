...
function testPrimitive(name, createFunction, params = {}) {
  ...
  try {
    ...
    if (mesh.vertices.length > 0) {
      ...
      console.log(
        `  ✅ Vertex moved from ${JSON.stringify(originalPosition)} to ${JSON.stringify(updatedVertex.position)}`
      );
      ...
      console.log(
        `  ✅ Geometry conversion successful: ${geometry.attributes.position.count} positions`
      );
    }
