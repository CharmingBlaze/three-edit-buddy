...
function findVertexGroups(mesh: EditableMesh, threshold: number): number[][] {
  ...
  for (const vertex of mesh.vertices) {
    ...
    for (const otherVertex of mesh.vertices) {
      if (otherVertex.id === vertex.id || processed.has(otherVertex.id)) continue;
...
function mergeVertexGroup(
  ...
  mesh.edges = mesh.edges.filter(edge => {
    const hasRemovedVertex = edge.vertexIds.some(id => verticesToRemove.includes(id));
    if (hasRemovedVertex) {
 ...
 */
export function mergeSpecificVertices(mesh: EditableMesh, vertexIds: number[]): number {
