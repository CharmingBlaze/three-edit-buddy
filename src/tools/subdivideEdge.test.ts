...
function makeMesh(vertices, edges, faces = []) {
  return {
    ...
    faces: faces.map((f, i) => ({ id: i, vertexIds: f, edgeIds: edges.map((_, j) => j), materialId: undefined, uvIds: [] })),
    materials: [],
    uvs: [],
...
describe('subdivideEdges', () => {
  it('subdivides multiple edges and returns new vertex IDs', () => {
    ...
    expect(mesh.vertices[newVertexIds[0]].position).toEqual({ x: 1, y: 0, z: 0 });
    expect(mesh.vertices[newVertexIds[1]].position).toEqual({ x: 1, y: 2, z: 0 });
  });
});
