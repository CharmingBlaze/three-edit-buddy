...
export function bridgeEdges(
  ...
  for (let i = 0; i < minVertices; i++) {
    ...
    if (i < vertices1.length && nextI < vertices1.length && 
        i < vertices2.length && nextI < vertices2.length) {
 ...
 */
function getVerticesFromEdgeLoop(mesh: EditableMesh, edgeLoop: number[]): number[] {
