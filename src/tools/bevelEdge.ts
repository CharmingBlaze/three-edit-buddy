...
export function bevelEdge(
  ...
  for (const edgeId of edgeIds) {
    ...
    for (const face of connectedFaces) {
      ...
      // Add new vertices
      const newVertexId1 = builder.addVertex(offset1, `bevel-${v1.id}-${face.id}`);
      const newVertexId2 = builder.addVertex(offset2, `bevel-${v2.id}-${face.id}`);
      
      // Create bevel face (quad)
      const bevelFaceId = builder.addQuad([v1.id, newVertexId1, newVertexId2, v2.id]);
