...
export function extrudeFaces(
  ...
  for (const [normalKey, groupFaceIds] of faceGroups) {
    ...
    for (const faceId of groupFaceIds) {
      const extrudedFaces = extrudeSingleFace(
        mesh,
        builder,
        faceId,
        normal,
...
function extrudeSingleFace(
  ...
  if (vertexCount === 3) {
    ...
    const newFaceId = builder.addTriangle(triangle, `extruded-face-${faceId}`);
    ...
    const newFaceId = builder.addQuad(quad, `extruded-face-${faceId}`);
    newFaceIds.push(newFaceId);
  } else {
...
function calculateFaceNormal(mesh: EditableMesh, face: Face): Vector3Like {
  const vertices = face.vertexIds.map(
    (id) => mesh.vertices.find((v) => v.id === id)!
  );
