...
export function UVSelectionHighlight(
  ...
  if (showVertices && selection.selectedVertices.size > 0) {
    ...
    if (positions.length > 0) {
      vertexGeometry.setAttribute('position', new Float32BufferAttribute(positions, 3));
...
function extractUVData(mesh: EditableMesh): UVData | null {
  ...
  for (const vertex of mesh.vertices) {
    const x = Array.isArray(vertex.position) ? vertex.position[0] : vertex.position.x;
    const y = Array.isArray(vertex.position) ? vertex.position[1] : vertex.position.y;
