...
class PrimitiveDemo {
  ...
  createPrimitive(primitiveName) {
    ...
    console.log(
      `Primitive added to scene. Scene children:`,
  ...
  setupEditableMeshTopology(editableMesh) {
    ...
    this.faces = editableMesh.faces.map((face) => ({
      id: face.id,
      vertexIds: face.vertexIds.map((originalId) =>
        vertexIdMap.get(originalId)
      ),
    }));

    this.edges = editableMesh.edges.map((edge) => ({
      id: edge.id,
      vertexIds: edge.vertexIds.map((originalId) =>
        vertexIdMap.get(originalId)
      ),
    }));
    ...
    this.vertices.forEach((vertex) => {
      vertex.connectedFaces = this.faces
        .filter((face) => face.vertexIds.includes(vertex.id))
        .map((face) => face.id);
    });
  ...
  createEdgeLine(v1, v2) {
    ...
    lineGeometry.setAttribute(
      'position',
      new THREE.Float32BufferAttribute([v1.x, v1.y, v1.z, v2.x, v2.y, v2.z], 3)
    );
  ...
  updatePrimitiveInfo(primitiveName) {
    ...
    nameElement.textContent =
      primitiveName.charAt(0).toUpperCase() + primitiveName.slice(1);
  ...
  updateButtonStates(
    ...
    if (selectionMode) {
      ...
      if (faceBtn)
        faceBtn.classList.toggle('active', selectionMode === 'select-faces');
      if (edgeBtn)
        edgeBtn.classList.toggle('active', selectionMode === 'select-edges');
    }
  }
  ...
  moveVertex(vertexId, newPosition) {
    if (vertexId >= 0 && vertexId < this.vertices.length) {
      ...
      if (this.currentMesh?.userData?.editableMesh) {
        ...
        this.currentMesh.userData.editableMesh.moveVertex(
          originalVertexId,
          newPosition
        );
        // Update the Three.js geometry
        this.currentMesh.geometry.dispose();
        this.currentMesh.geometry =
          this.currentMesh.userData.editableMesh.toBufferGeometry();
      } else {
  ...
  onMouseMove(event) {
    ...
    if (intersection) {
      ...
      this.updateGeometryVertex(
        this.selectedVertex.userData.vertexId,
        intersection
      );
    }
  }
