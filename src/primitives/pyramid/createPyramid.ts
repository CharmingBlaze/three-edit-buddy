...
export function createPyramid(params: PyramidParams = {}): EditableMesh {
  ...
  const vertices = [
    // Base
    { id: mesh.addVertex({ x: -halfSize, y: -halfHeight, z: -halfSize }).id, position: { x: -halfSize, y: -halfHeight, z: -halfSize } }, // 0
    { id: mesh.addVertex({ x: halfSize, y: -halfHeight, z: -halfSize }).id, position: { x: halfSize, y: -halfHeight, z: -halfSize } },  // 1
    { id: mesh.addVertex({ x: halfSize, y: -halfHeight, z: halfSize }).id, position: { x: halfSize, y: -halfHeight, z: halfSize } },   // 2
    { id: mesh.addVertex({ x: -halfSize, y: -halfHeight, z: halfSize }).id, position: { x: -halfSize, y: -halfHeight, z: halfSize } },  // 3
    // Top
    { id: mesh.addVertex({ x: 0, y: halfHeight, z: 0 }).id, position: { x: 0, y: halfHeight, z: 0 } }                  // 4
  ];
  
  // Create faces
  mesh.addFace([vertices[0]!.id, vertices[1]!.id, vertices[2]!.id, vertices[3]!.id], []); // Base
